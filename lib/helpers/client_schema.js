...
module.exports = function getSchema(provider) {
  ...
  const DEFAULT_CONFIGURATION = _.clone(configuration.clientDefaults);
  ...
  if (features.request.enabled || features.requestUri.enabled) {
  ...
  if (features.requestUri.enabled) {
    ...
    constructor(metadata) {
      ...
      if (this.default_max_age !== undefined) {
        if (!Number.isSafeInteger(this.default_max_age) || this.default_max_age < 0) {
          invalidate('default_max_age must be a non-negative integer');
        }
      }
      ...
      clientAuthEndpoints.forEach((endpoint) => {
        if (this[`${endpoint}_endpoint_auth_method`] === 'tls_client_auth') {
          if ('tls_client_auth_san_dns' in this) {
            invalidate('tls_client_auth_san_dns is not supported');
          }
          if ('tls_client_auth_san_uri' in this) {
            invalidate('tls_client_auth_san_uri is not supported');
          }
          if ('tls_client_auth_san_ip' in this) {
            invalidate('tls_client_auth_san_ip is not supported');
          }
          if ('tls_client_auth_san_email' in this) {
            invalidate('tls_client_auth_san_email is not supported');
          }
